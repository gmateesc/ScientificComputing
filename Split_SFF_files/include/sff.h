...
/*
 * The struct for creating a fastq file
 */
typedef struct {      
  char  *name;
  char  *bases;
...
void convert_big_endian_read_header_2_host(sff_read_header *rh);
...
void write_sff_read_data(FILE *fp, 
                   sff_read_data *rd, 
                   uint16_t nflows, 
		   uint32_t nbases, 
		   uint32_t read_num);
...
void free_fastq(struct_fastq *fq);
...
void bailout(FILE *fp, char * msg, int err);
